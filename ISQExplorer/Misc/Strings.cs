...
namespace ISQExplorer.Misc
{
    public static class Strings
    {
        ...
        /// <param name="number">Return the nth capture group. By default this is 1 which gets the first capture group.</param>
        ...
        public static Optional<string> Capture(this string input, string pattern, int number = 1)
        {
            ...
            return matches.Count <= number ?
                new Optional<string>() :
                matches.Skip(number).First().Value;
        }
        ...
        public static string Join(this IEnumerable<string> enumerable, string delimiter) => string.Join(delimiter, enumerable);
    }
}